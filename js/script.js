...
const totalPages = data.products.length / 3;
...
function generatePagination(currentPage) {
  ...
  paginationHTML += "</ul>";
  let firstCut = (currentPage - 1) * pageSize;
  let secondCut = firstCut + pageSize;
  renderTable(firstCut, secondCut);
...
function handlePageClick(event) {
  ...
  if (page === "prev") {
    ...
    let prevElement = currentPageItem.previousElementSibling;

    if (prevElement) {
      let prevElementNumber = parseInt(prevElement.innerText);
      currentPageItem.classList.remove("active");
      prevElement.classList.add("active");
      currentPage = prevElementNumber;
      if (currentPage === 1) {
  ...
  //next page
    ...
    let currentPageNumber = parseInt(currentPageItem.innerText);
    let nextPageItem = currentPageItem.nextElementSibling;
    if (nextPageItem) {
      let nextPageNumber = parseInt(nextPageItem.innerText);
      currentPageItem.classList.remove("active");
    ...
    let firstCut = (currentPageNumber - 1) * pageSize;
    let secondCut = firstCut + pageSize;
  ...
  //handling pages
    ...
    if (page) {
      ...
      let pageLink = document.querySelectorAll(".page-item");
      pageLink.forEach((item) => {
        if (currentPage.toString() === item.innerText) {
          item.classList.add("active");

          if (item.innerText === "1") {
            const prev = pageLink[0];
            prev.classList.add("disabled");
          }

          if(item.innerText === "10") {
            const next = pageLink[pageLink.length - 1];
...
paginationContainer.innerHTML = generatePagination(currentPage);

// Add event listener for page click
paginationContainer.addEventListener("click", handlePageClick);

function renderTable(firstCut, secondCut) {
  tbody.innerHTML = "";
  tbody.innerText = "";
  let tableData = data.products.slice(firstCut, secondCut);
  tableData.forEach((item, index) => {
    let row = document.createElement("tr");
    let tableId = document.createElement("td");
    tableId.innerText = item.id;
    ...
    tableDescription.innerText = item.description;
    row.appendChild(tableDescription);

    let tablePrice = document.createElement("td");
    tablePrice.innerText = `₹ ${item.price}`;
    row.appendChild(tablePrice);

    let tableDiscount = document.createElement("td");
    tableDiscount.innerText = item.discountPercentage;
    row.appendChild(tableDiscount);

    let tableRating = document.createElement("td");
    tableRating.innerText = item.rating;
    row.appendChild(tableRating);

    let tableStock = document.createElement("td");
    tableStock.innerText = item.stock;
    row.appendChild(tableStock);

    let tablebrand = document.createElement("td");
    tablebrand.innerText = item.brand;
    row.appendChild(tablebrand);

    let tablecategory = document.createElement("td");
    tablecategory.innerText = item.category;
    row.appendChild(tablecategory);

    let tableThumbnail = document.createElement("td");
    const thumbnailImg = document.createElement("img");
    thumbnailImg.setAttribute("src", item.thumbnail);
    thumbnailImg.setAttribute("style", "width:200px");
    tableThumbnail.appendChild(thumbnailImg);
    row.appendChild(tableThumbnail);
